...
public class CRCUtil {
	...
	public static int calcCRC(byte[] data){//校验数组data
	...
	public static int calcCRC(byte[] data,int offset,int len){
		return calcCRC(data,offset,len,0xffff);//0xffff类似一个秘钥
	}
	public static int calcCRC(byte[] data,int offset,int len,int preval){//offset管道为0
		int ucCRCHi =(preval & 0xff00)>>8; //&运算后右移8位
		int ucCRCLo = preval& 0x00FF; //&运算
		int iIndex;
		for(int i=0;i<len;i++){
			iIndex = (ucCRCLo^data[offset+i]) &0x00FF;//iIndex = lo^(异或)字节数组第i位 再&(与)0x00FF
			ucCRCLo = ucCRCHi^t_crc16_h[iIndex];//lo = hi ^ t_crc16_h 第iIndex位
			ucCRCHi = t_crc16_l[iIndex];//hi = t_crc16_l 第iIndex位
		}
		return ((ucCRCHi&0x00FF)<<8)|(ucCRCLo&0x00FF) &0xFFFF;// hi & 0x00FF <<8  |  lo &0x00FF   中的 再 &0x00FF
	}
